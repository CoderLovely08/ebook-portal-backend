...
export class BookService {
    ...
    static async createBook(bookData) {
        try {
            const { categories: categoryIds, ...restBookData } = bookData;
            ...
            await SupabaseService.supabaseDeleteFile(bookData.coverImage);
            await SupabaseService.supabaseDeleteFile(bookData.filePath);
        ...
        try {
            ...
            if (error.code === PRISMA_ERROR_CODES.P2025.code) {
                throw new CustomError(
                    "Book or one or more categories not found",
                    400
                );
            }
    ...
    static async updateBookFilePath(id, filePath) {
        try {
            ...
            throw new CustomError(
                `Error updating book file path: ${error.message}`
            );
        }
    }
}
